...
def obtener_fechas():
    conn = obtener_conexion()
    cursor = conn.cursor()
    cursor.execute('SELECT MIN(fechapublicacion), MAX(timestamp) FROM avisos_processed')
...
def obtener_avisos_paginado(pagina, fecha_filtro=None, categoria_filtro=None, texto_filtro=None):
    ...
    base_query = f'SELECT {columnas_seleccionadas} FROM avisos_processed'
    count_query = 'SELECT COUNT(*) FROM avisos_processed'
